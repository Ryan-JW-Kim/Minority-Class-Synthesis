...
def execute(data_key):
	...
	with torch.no_grad():    
		...
		y = torch.from_numpy(y_train).to(device).float().unsqueeze(1)
		mu, logvar = cvae.encode(x, y)
		z = cvae.reparameterize(mu, logvar)
	...
	for _ in range(5):
		...
		# Execute optimization upon the original + synthetic data
		...
		new_x_train = np.concatenate((x_train, synthesized_features), axis=0)
		new_y_train = np.concatenate((y_train, [minority_labels[0]] * len(synthesized_features)), axis=0)
	...
	return result, synthesized_features
	
if __name__ == "__main__":
	timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
	print(f"Started program: {timestamp}")

	for data_key in data_mapper:
		try:
		
			start_time = time.time()
			# if "yeast4" not in data_key:
			# 	continue

			if os.path.exists(f"results/{data_key}.result"):
				continue

			result, synthetic_samples = execute(
				data_key
			)
			
			with open(f"results/{data_key}.result", 'wb') as fh:
				pickle.dump(result, fh)
			
			pd.DataFrame(synthetic_samples).to_csv(f'results/{data_key}.csv', index=False)

			print(f"Completed {data_key} in: {round(time.time() - start_time // 60, 2)} minutes")
